//
//  BottomsListViewController.swift
//  GeekProject
//
...
class BottomsListViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,ClothesInfoTableViewCellDelegate {

    // カテゴリナンバーを２にする
    let category = 2
    
    var bottomses = [Bottoms]()
    ...
    @IBOutlet var clothesListTableView : UITableView!
        ...
        func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            let clothesImagePath = bottomses[indexPath.row].imageUrl
            clothesImageView.kf.setImage(with: URL(string: clothesImagePath))
            //　他の情報を表示させる
            cell.brandNameLabel.text = bottomses[indexPath.row].brandName
            cell.buyDateLabel.text = bottomses[indexPath.row].buyDate
            cell.priceLabel.text = bottomses[indexPath.row].price
            cell.commentTextView.text = bottomses[indexPath.row].comment
            cell.putOnCountLabel.text = String(bottomses[indexPath.row].putOnCount)
            
            return cell
        }
        
        func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
            self.performSegue(withIdentifier: "editBottoms", sender: nil)
        ...
        //buttonに、押されたボタンのタグ番号（イント）が保存されているので、longTopses配列のタグ番目の着用回数を扱う
            //データの取り出しと更新
            var putOnCount =  self.bottomses[button.tag].putOnCount
            putOnCount = putOnCount + 1
            
            let query = NCMBQuery(className: "bottoms")
            query?.getObjectInBackground(withId: bottomses[button.tag].objectId, block: { (result, error) in
                if error != nil {
        ...
        @objc func didTapEditButton(tableViewCell: UITableViewCell, button: UIButton) {
            ...
            let deleteAction = UIAlertAction(title: "削除", style: .default) { (action) in
                //ボタンを押されたセルのデータを呼び出し
                let query = NCMBQuery(className: "bottoms")
                query?.getObjectInBackground(withId: self.bottomses[button.tag].objectId, block: { (result, error) in
                    if error != nil {
                        KRProgressHUD.showError()
                    } else {
                        // データの削除
            ...
            query?.findObjectsInBackground({ (result, error) in
                if error != nil  {
                    ...
                    for addObject in result as! [NCMBObject] {
                      ...
                      // 上の二つのデータを合わせてBottomsクラスにセット
                        let bottoms = Bottoms(objectId: addObject.objectId, user: userModel, imageUrl: imageUrl, brandName: brandName, buyDate: buyDate, comment: comment, price: price,putOnCount: putOnCount,colorNumber: colorNumber)
                        
                      // 配列に加える
                        self.bottomses.append(bottoms)
        ...
        @IBAction func addClothes () {
            self.performSegue(withIdentifier: "bottomsSegue", sender: nil)
        }
        // カテゴリナンバーを送る
        override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
            // データの追加の場合
            if segue.identifier == "bottomsSegue" {
                ...
                // データの更新の場合
            } else if segue.identifier == "editBottoms" {
                let editViewController = segue.destination as! EditViewController
                let selectedIndex = clothesListTableView.indexPathForSelectedRow!
                editViewController.objectId = bottomses[selectedIndex.row].objectId
                editViewController.colorNumber = bottomses[selectedIndex.row].colorNumber
                editViewController.imageUrl = bottomses[selectedIndex.row].imageUrl
                editViewController.brandName = bottomses[selectedIndex.row].brandName
                editViewController.buyDate = bottomses[selectedIndex.row].buyDate
                editViewController.price = bottomses[selectedIndex.row].price
                editViewController.comment = bottomses[selectedIndex.row].comment
